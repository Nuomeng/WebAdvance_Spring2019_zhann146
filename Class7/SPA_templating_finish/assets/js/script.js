...
app.main = (function() {
  ...
  function attachEvents() {
    ...
    checkboxes.click(function() {
      ...
      var that = $(this), //点击的那个元素
        cat = that.attr('name'); // 取到json所在元素的name
      ...
      if (that.is(":checked")) {
        ...
        createQueryHash(filters); // call function change the hash tag
      }
      if (!that.is(":checked")) {
  ...
  /*------------------------------------------------*/
  // get  the filters obkect, turn it into a string and wirte it into the hash
  /*------------------------------------------------*/
  function createQueryHash(filters) {

    if (!$.isEmptyObject(filters)) {
      var str = JSON.stringify(filters); // converts a JavaScript object or value to a JSON string,
  ...
  /*------------------------------------------------*/
  //this is going to filter the componets everytime you select a checkbox,depending on the filter
  /*------------------------------------------------*/

  function renderFilterResylts(filters, students) {
    ...
    checkboxes.prop('checked', false); // method gets the property value for only the first element in the matched set
      ...
      if (filters[c] && filters[c].length) {
        ...
        filters[c].forEach(function(filter) {
          ...
          if (c && filter) {
            $('input[name=' + c + '][value = '+filter+' ]').prop('check',true);
          }
